...
esp_err_t radio_init(void)
{
    ...
    const uart_port_t port = (uart_port_t)CONFIG_CANSAT_RADIO_UART_NUM;
    const uart_config_t config = {
        .baud_rate = CONFIG_CANSAT_RADIO_BAUD,
    ...
    if (result == ESP_OK) {
        result = uart_set_pin(port, CONFIG_CANSAT_RADIO_TX_GPIO, CONFIG_CANSAT_RADIO_RX_GPIO,
...
esp_err_t radio_recover(void)
{
    ...
    if (s_driver_installed) {
        (void)uart_driver_delete((uart_port_t)CONFIG_CANSAT_RADIO_UART_NUM);
        s_driver_installed = false;
    }
    return radio_init();
}
...
esp_err_t radio_send(const char *data, size_t length, uint32_t timeout_ms)
{
    ...
    if (written < 0 || (size_t)written != length) return ESP_FAIL;
...
int radio_read(uint8_t *buffer, size_t capacity, uint32_t timeout_ms)
{
    ...
    return uart_read_bytes((uart_port_t)CONFIG_CANSAT_RADIO_UART_NUM, buffer,
